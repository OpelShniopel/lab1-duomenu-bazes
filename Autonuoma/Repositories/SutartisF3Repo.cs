...
public class SutartisF3Repo
{
	public static List<SutartisL> ListSutartis()
	{
		var query =
			...
			FROM
				`sutartys` s
				LEFT JOIN `darbuotojai` d ON s.fk_darbuotojas=d.tabelio_nr
				LEFT JOIN `klientai` n ON s.fk_klientas=n.asmens_kodas
				LEFT JOIN `sutarties_busenos` b ON s.busena=b.id
			ORDER BY s.nr DESC";
	...
	{
		var query = $@"SELECT * FROM `sutartys` WHERE nr=?nr";
		var drc =
	...
	{
		var query = $@"DELETE FROM `sutartys` where nr=?nr";
	...
	{
		var query = $@"SELECT * FROM `sutarties_busenos` ORDER BY id ASC";
		var drc = Sql.Query(query);
	...
	public static List<UzsakytaPaslaugaM> ListUzsakytaPaslauga(int sutartisId)
	{
		var query =
			...
			FROM 
				`uzsakytos_paslaugos` AS up
				LEFT JOIN `paslaugos` AS p 
					ON up.fk_paslauga=p.id
				LEFT JOIN `paslaugu_kainos` as pk
